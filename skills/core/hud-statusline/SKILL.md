---
name: hud-statusline
description: "Configures Codex's statusline to display Unity workflow state — current phase, active agent, files modified, and session duration."
---

# HUD / Statusline Integration

Guidance for commands and agents to update Codex's statusline with real-time Unity workflow state.
...
Statusline data comes from the hook state directory at `/tmp/unity-codex-hooks/`:
...
```bash
EDIT_COUNT=0
if [ -f "/tmp/unity-codex-hooks/session-edits.txt" ]; then
    EDIT_COUNT=$(sort -u "/tmp/unity-codex-hooks/session-edits.txt" | wc -l | tr -d ' ')
fi
```
...
```bash
DURATION_MINS=0
if [ -f "/tmp/unity-codex-hooks/session-start-time" ]; then
    START=$(cat "/tmp/unity-codex-hooks/session-start-time")
    NOW=$(date +%s)
...
```bash
CALL_COUNT=0
if [ -f "/tmp/unity-codex-hooks/session-cost.jsonl" ]; then
    CALL_COUNT=$(wc -l < "/tmp/unity-codex-hooks/session-cost.jsonl" | tr -d ' ')
fi
```
