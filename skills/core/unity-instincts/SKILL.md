...
Our static rules in `.codex-legacy/rules/` encode *universal* Unity truths (serialization safety, lifecycle order, perf golden rules). But every project has its own conventions the rules can't know:
...
- For cross-cutting project decisions (architecture, pipeline choice) — those belong in AGENTS.md.

Instincts fill the gap between "obvious universal" and "deliberate project policy".

## Commands

All user-facing operations are in `/unity-instincts` (see `.codex-legacy/commands/unity-instincts.md`). Hooks are automatic and silent.
