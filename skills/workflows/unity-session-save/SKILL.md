...
description: "Save the current session state as a labeled snapshot to .codex-unity/state/sessions/<label>.json for later /unity-session-resume."
---

# /unity-session-save — Snapshot Current Session

Save the live session state (`.codex-unity/state/session.json`) as a named snapshot: **$ARGUMENTS**
...
2. Check that `.codex-unity/state/session.json` exists. If not, ask the user whether to save the in-memory context (in which case trigger a Stop-hook-equivalent save first, or tell them to wait until at least one Edit/Read has happened).
3. Ensure `.codex-unity/state/sessions/` exists.
4. Read `session.json`; inject a top-level `label` field and a `source: "manual-save"` field into a copy.
5. Write to `.codex-unity/state/sessions/<label>.json`.
