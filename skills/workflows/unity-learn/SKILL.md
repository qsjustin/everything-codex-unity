...
This command works with the data collected by the `auto-learn.sh` hook (strict profile) which records session patterns to `.codex-unity/state/learnings.jsonl` after each session. For pre-v1.3.0 projects, the file may be at `.codex-unity/state/learnings.jsonl` instead.

## Subcommands

### `review` (default)

Read `.codex-unity/state/learnings.jsonl` and present a dashboard summarizing accumulated data:

1. **Read the learnings file** at `.codex-unity/state/learnings.jsonl` (or `.codex-unity/state/learnings.jsonl` as fallback for pre-v1.3.0 projects)
...
1. **Read all entries** from `.codex-unity/state/learnings.jsonl` (or `.codex-unity/state/learnings.jsonl` as fallback)
...
4. **Output** the draft to stdout with instructions:
   ...
   Draft skill generated. To install:
     1. Create directory: skills/core/[skill-name]/
     2. Save the above content to: skills/core/[skill-name]/SKILL.md
...
1. **Read all entries** from `.codex-unity/state/learnings.jsonl` (or `.codex-unity/state/learnings.jsonl` as fallback for pre-v1.3.0 projects)
2. **Present:**
...
- **No data, no output** — if `.codex-unity/state/learnings.jsonl` (or `.codex-unity/state/learnings.jsonl`) doesn't exist or is empty, say so clearly
