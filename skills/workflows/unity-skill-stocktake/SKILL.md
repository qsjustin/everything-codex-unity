...
Scan `skills/` and `.codex-legacy/agents/` for hygiene issues. This is a read-only audit — the output is a report the user can act on.

## Steps

### 1. Enumerate entries

- Skills: `skills/**/SKILL.md` (recursive; categorized by parent directory).
- Agents: `.codex-legacy/agents/*.md`.
- Commands: `.codex-legacy/commands/*.md` (for cross-reference when checking whether skills are referenced).
...
If `.codex-unity/state/learnings.jsonl` or `.codex-unity/state/instincts/observations.jsonl` exist, check whether each skill has been mentioned or auto-loaded in the past N sessions (N = 10 by default). Skills with zero recent loads are candidates for demotion or deletion.
...
- File paths (`.codex-legacy/...` or `Assets/...`) that no longer exist.
