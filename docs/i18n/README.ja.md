...
**Unity ゲーム開発のための究極の Codex ツールキット。**

Codex に Unity の深い専門知識を与える、本番環境対応のプラグ&プレイシステムです。パフォーマンスの高い C# の記述からシーン構築、パフォーマンスプロファイリング、iOS/Android ビルドのトリガーまで、すべて自然言語で操作できます。
...
メカニクスを説明するだけで、Codex が C# スクリプトの記述、MCP によるシーン構築、物理レイヤーの設定、カメラの設定を行い、コンパイルが通ることを確認します。
...
### 前提条件
- [Codex](https://openai.com/codex) がインストール済み
- Unity 2021.3 LTS 以降
...
MCP ブリッジにより、Codex が Unity エディタを直接操作できるようになります。シーン構築、プロファイリング、ビルドなどが可能です。
...
### プロジェクトライフサイクル
```
/unity-init                     プロジェクトのスキャン＋AGENTS.md の自動生成
...
./scripts/generate-agents-md.sh > AGENTS.md   # プロジェクト用 AGENTS.md の自動生成
```
...
- `examples/AGENTS.md.hyper-casual` — ワンタップ操作、ミニマルなビジュアル、広告マネタイズ
- `examples/AGENTS.md.match3` — グリッドシステム、カスケード、スペシャルタイル、ライフ/エナジー
- `examples/AGENTS.md.idle-clicker` — 巨大な数値、オフライン進行、プレステージシステム
- `examples/AGENTS.md.mobile-casual` — タッチ入力、小さなビルドサイズ、広告連携
- `examples/AGENTS.md.2d-platformer` — Tilemap、バーチャルジョイスティック、モバイル最適化
- `examples/AGENTS.md.rpg` — ステータス、インベントリ、ダイアログ、タッチ操作
...
```
Codex が PlayerView.cs を編集しようとする
    |
...
Stop
    ...
    +-->  session-save.sh -- /tmp/unity-codex-hooks/ に状態を保存
